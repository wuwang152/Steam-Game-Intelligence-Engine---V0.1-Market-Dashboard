...
|---|---|
| Home | `summary_metrics` + system figures |
| 市场结构 | `yearly_release_counts`, `price_bucket_distribution`, `owners_tier_distribution`, `platform_count_distribution` |
| 口碑与热度 | `review_signal_distribution`, `review_sentiment_distribution`, `review_bucket_positive_rate`, `top_games_by_reviews`, `top_rated_games` |
| 赛道与本地化 | `genre_distribution`, `tag_distribution`, `category_distribution`, `language_support_summary`, `localization_by_genre` |
| 机会识别 | `hidden_gems`, `low_price_high_rating`, `high_attention_low_rating`, `chinese_supported_potential`, `localization_opportunities` |
